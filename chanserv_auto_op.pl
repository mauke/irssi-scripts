...
sub hook_op_signal {
	...
	Irssi::signal_add_first $sig => sub {
		...
		#print esc "[$IRSSI{name}] intercepting $sig \@ $channel->{name}\n" . Dumper(@_);
...
Irssi::signal_add_last 'nick mode changed' => sub {
	...
	if ($pendant->{state} == REQUESTING_OPS && $channel->{chanop}) {
		...
		for my $spec (@{delete $pendant->{signals}}) {
			#print esc "[$IRSSI{name}] resending\n" . Dumper(@$spec);
			&Irssi::signal_emit(@$spec);
		}
	}
};
...
hook_op_cmd 'mode' => sub {
	my ($args, $server, $witem) = @_;
	$args =~ /^\s*(?:([#&!]\S+)\s+)?[+-](?:\S*[cegiImnpQrRstz]|\S+\s+\S)/ && ($1 ? $server->channel_find($1) : $witem)
};
